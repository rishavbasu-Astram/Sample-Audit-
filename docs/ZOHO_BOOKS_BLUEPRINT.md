...
| 9 | **Email & Document Ingestion (AI Auto-Capture)** | Forward/watch an inbox → Claude extracts the data → matches customers/vendors/items → creates a **draft** quote/bill/expense/PO for one-click review. Ingress channels, dedup, AI extraction, matching, review queue, feedback loop, guardrails |
